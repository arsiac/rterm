//! 终端按键 / 鼠标绑定。
//!
//! 把原始输入（字符、功能键、鼠标键）按修饰键与终端模式匹配到高层
//! [`BindingAction`]，供终端部件在事件循环中派发。
...
pub enum BindingAction {
    ...
    /// 向终端发送单个字符（如 `a`、`\x0d`）。
    Char(char),
    /// 向终端发送一段转义序列（如功能键对应的 CSI 序列）。
    Esc(String),
    /// 打开光标下的超链接（Hyperlink / OSC 8）。
    LinkOpen,
    /// 不命中的默认动作（透传输入）。
    Ignore,
}
...
/// 一条绑定：将「输入 + 修饰键 + 终端模式」映射到动作。
pub struct Binding<T> {
    /// 触发该绑定的输入。
    pub target: T,
    /// 必须同时满足的修饰键集合（为空表示不要求）。
...
/// 键盘绑定（`target` 为 [`InputKind`]）。
pub type KeyboardBinding = Binding<InputKind>;
/// 鼠标绑定（`target` 为 [`InputKind`]）。
pub type MouseBinding = Binding<InputKind>;

/// 声明式生成绑定表的宏：以 `(输入[, 修饰键][,+模式][,~模式]; 动作)` 逐行描述绑定。
#[macro_export]
...
/// 绑定表布局：持有已解析的「绑定 → 动作」规则列表，供终端部件查询命中动作。
#[derive(Clone, Debug)]
pub(crate) struct BindingsLayout {
    /// 绑定规则列表（每条含输入、修饰键、终端模式与对应动作）。
...
impl BindingsLayout {
    /// 构造默认绑定表：合并内置键盘绑定、平台专属绑定与鼠标默认绑定。
    ...
    /// 追加或覆盖绑定：输入完全一致的条目以新动作替换，否则追加到列表末尾。
    ...
    /// 按输入、修饰键与终端模式匹配已注册绑定，命中返回对应动作，否则返回 `Ignore`。
...
/// 内置键盘绑定表：覆盖普通键、应用光标模式、Ctrl / Shift / Alt 及其组合修饰键。
fn default_keyboard_bindings() -> Vec<(Binding<InputKind>, BindingAction)> {
    generate_bindings!(
        ...
        Insert,     Modifiers::ALT; BindingAction::Esc("\x1b[3;3~".into());
...
/// macOS 平台专属键盘绑定：Command+C / Command+V 复制粘贴、Command+H 退格。
#[cfg(target_os = "macos")]
...
/// 非 macOS 平台键盘绑定：Command+Shift+C / Command+Shift+V 复制粘贴。
...
/// 鼠标默认绑定：Command+左键打开光标下的超链接（OSC 8）。
...
#[cfg(test)]
mod tests {
    use crate::widget::term::bindings::MouseBinding;
