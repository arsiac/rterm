...
pub enum TransferStatus {
    ...
    ///
    /// **刻意独立于 [`Self::Queued`]**：退避期间不占并发额度、不参与调度序，故不能复用
    /// 「排队中」——那会让面板把它算进排队数、并让调度器误以为它可以立即启动。到点后由
    /// `Message::RetryDue` 转回 `Queued`。
    WaitingRetry,
    /// 已完成。
    Done,
    /// 失败 / 已取消（含自动重试次数耗尽）。
...
///
/// 传输按「全局并发 N」调度，多个任务可同时持有同一 SFTP 客户端（调度与续传机制见
/// `app::transfer` 模块文档）。失败后按 `[transfer] retry_attempts` 指数退避自动重试，
/// 源端指纹未变时下一轮从上次落盘的字节继续（见 [`Transfer::resume`]）。
#[derive(Clone)]
pub struct Transfer {
    ...
    /// 成功、行重新跑起来就回到干净样子，否则一次网络抖动会留下永久徽标。
    /// 故该字段在成功路径上**不清零**（重试预算要用它），面板只是不显示它。
    pub attempts: u32,
    ...
    ///
    /// 清理只发生在取消 / 移除行 / 关标签三处，失败行保留 `.part` 作为续传起点。清理失败时把
    /// 路径留在这里提示用户手动处理——只有下载侧会走到这一步，上传侧的远端残留只记日志。
    ...
    /// 这一份，否则会话重建后会排队在已失效的通道上。`None` 只出现在边界情形与无需真实连接的单测里。
