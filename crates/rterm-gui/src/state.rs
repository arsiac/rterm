...
///
/// 传输按「全局并发 N」调度（跨标签、跨方向共享额度）：同一条 SFTP 通道可承载多个请求，
/// russh-sftp 的会话内部按请求 id 多路复用回执、操作全为 `&self`，故多个任务可同时持有同一
/// 客户端（调度细节见 `app::transfer`）。
///
/// 速度由 `run_transfer` 的 stream 任务按真实 I/O 间隔估算并随进度消息携带（核心层只回传累计
/// 字节）。失败后按 `[transfer] retry_attempts` 指数退避自动重试，**下一轮从上次落盘的字节继续**
/// （源端指纹未变时，见 `Transfer::resume`）；半成品只在取消 / 移除 / 关标签时清理，失败行两侧
/// 都保留它作为续传起点。
#[derive(Clone)]
pub struct Transfer {
    ...
    ///
    /// 清理只发生在**取消、移除行与关标签**三处；两侧的失败行都不清理，那个 `.part` 是续传的
    /// 起点，行上「继续下载 / 继续上传」就是冲它去的。清理失败时把路径留在这里提示用户手动
    /// 处理——只有下载侧会走到这一步，上传侧的远端残留只记日志（重传会截断重写它）。
    ...
    pub speed: f64,
    /// 该传输**入队时**捕获的 SFTP 客户端（回退用；权威值始终是标签此刻的那一份）。
    ///
    /// 启动 / 重试优先用 `Ctx::client_for` 注入的当前客户端，只在标签此刻拿不到客户端时才回退到
    /// 这一份。会话重建后同一标签已换上新通道，直接用这份记录会把排队项与手动重试打在旧会话上。
    /// `None` 只出现在「客户端已失效 / 尚未建立」的边界情形与无需真实连接的单测里。
    ...
    /// 校验基准（判定见 `app::transfer::resume_offset`）。失败与被取消**不清**它——那正是续传
    /// 要用的东西。
    pub resume: Option<ResumeState>,
    /// 该行的暂存文件里是**完整数据**（内容已落全、只是改名失败），故任何清理路径都必须跳过它：
    /// 它不是半成品，而是用户唯一的一份数据。由 `Failure::keep_staging` 在最终失败时落到这里。
